...
local aceOptions = {
    ...
    args = {
        ...
        showOnlyQuestHolders = {
            type = "toggle",
            name = L["Hide characters with no progress"],
            desc = L["Toggles the display of characters that have have no progression on any shown raids."],
            order = 10, -- adjust based on your layout
            get = "ShouldShowOnlyQuestHolders",
            set = "ToggleShowOnlyQuestHolders",
        },
        fitToScreen = {
            type = "toggle",
            width = "full",
...
function LibAceAddon:ShouldShowOnlyQuestHolders(info)
    return self.db.profile.showOnlyQuestHolders
end

function LibAceAddon:ToggleShowOnlyQuestHolders(info, value)
    self.db.profile.showOnlyQuestHolders = value
end
...
function LibAceAddon:OnInitialize()
    ...
    local currentVersion = GetCurrentDataVersion()
    if self.db.profile.dataVersion ~= currentVersion then
        KRaidSkipTracker.MigrateLegacyData(self.db.profile.allPlayersData)
        self.db.profile.dataVersion = currentVersion
    end
    ...
    if KRaidSkipTracker.Models and KRaidSkipTracker.Models.Player then
        KRaidSkipTracker.Initialize()
    else
        print("Initializing Legacy Data!")
        KRaidSkipTracker.InitializeLegacy()
    end
